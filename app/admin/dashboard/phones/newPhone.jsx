...
export const NewPhone = ({ setNewPhone,setFetchData }) => {
    ...
    const handleSubmit = async (e) => {
        try {
            ...
            const addPhone = await addDoc(collection(db, `raloc/logistics/phones`), data);

            if (addPhone?.id) {
                toast.success('New Phone Uploaded successfully!')
                setFetchData(true)
                setNewPhone(false)
            } else {
        ...
        <div className="fixed top-0 left-0 w-full h-svh bg-black/30 backdrop-blur-sm flex items-center justify-center transition duration-500 z-50">
            <form onSubmit={handleSubmit} className="p-6 bg-white rounded-lg shadow max-w-xl w-full">
                ...
                <div className="flex justify-between items-center mb-4">
                    ...
                    <button onClick={() => setNewPhone(false)} type="button" className="p-2 rounded-md bg-gray-100 hover:bg-red-600 hover:text-white text-black transition duration-500">
